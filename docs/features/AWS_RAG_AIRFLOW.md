...
### RAG Core (`packages/eliza-rag-ingestion/` — namespace `eliza_rag`)

Extracted from `src/services/rag/` into a standalone local Python package
(`eliza-rag-ingestion`) installable in any runtime: platform, Airflow, MCP
server, or external repos.

| Component | Status | Details |
|-----------|--------|---------|
| Document parser | **Done** | `eliza_rag.document_parser` — naive, docling, VLM, GPT-4o parsers. Handles PDF, DOCX, TXT, HTML, Markdown |
| Chunking | **Done** | `eliza_rag.chunking` — semantic, fixed, sentence strategies. Configurable chunk size + overlap |
| Embeddings | **Done** | `eliza_rag.embeddings` — OpenAI, local (sentence-transformers), **Bedrock Titan** (v1 + v2, sync retry with backoff) |
| Vector store | **Done** | `eliza_rag.vector_store` — AWS OpenSearch Serverless (SigV4), local Elasticsearch |
| Retrieval | **Done** | `eliza_rag.retrieval` — semantic search, keyword search, **hybrid with RRF scoring** |
...
### Terraform — Two Independent Projects

Split (2026-03-03) into two fully standalone Terraform projects:

**`terraform/platform/`** — Full Eliza platform on AWS ECS Fargate.
VPC (2 AZs, public/private subnets, NAT), ALB, EFS, Cloud Map, Secrets
Manager, IAM. All services as Fargate tasks: Postgres, Redis, Neo4j,
Elasticsearch, Logstash, App, Celery, Flower, Frontend.

**`terraform/rag-pipeline/`** — Standalone RAG pipeline (atomic, portable).
Own VPC (`10.1.0.0/16`), MWAA (Managed Airflow), S3 buckets (raw, processed,
DAGs), OpenSearch Serverless, API Gateway + WAF, IAM, Secrets Manager for
SharePoint credentials. Zero dependencies on the platform — can be extracted
to a separate repo for customer deployments.
...
|-----------|-------------|--------|
| **MWAA Terraform module** | Standalone in `terraform/rag-pipeline/` with own VPC, IAM, secrets | **Done** |
| **S3 buckets (Terraform)** | raw-documents, processed-documents, airflow-dags | **Done** |
| **OpenSearch Serverless (Terraform)** | Vector collection + VPC endpoint | **Done** |
| **API Gateway + WAF (Terraform)** | MCP server endpoint with rate limiting | **Done** |
| **SharePoint → S3 sync DAG** | Airflow DAG using Microsoft Graph API for incremental mirroring to S3 | V0 built, needs fixes (#2, #4, #6) |
| **Document classifier task** | Fargate task to classify files and route by doc metadata | V0 built, needs fixes (#7) |
| **Unstructured text preprocessor** | NLP cleaning + ranking for unstructured docs | V0 built |
| **Structured data preprocessor** | Schema validation + normalization for Excel/CSV | V0 built |
| **PPT preprocessor** | Slide-aware extraction: one chunk per slide + speaker notes | V0 built |
| **PDF/scanned preprocessor** | Enhancement to existing OCR parsers for scale | V0 built |
| **Embedding load balancer** | Fan out requests, handle Bedrock rate limits at 2TB scale | V0 built, needs fixes (#5, #14) |

### Retrieval Layer (Adapt Existing)

| Component | What Exists | What's Needed |
|-----------|-------------|---------------|
| **MCP tools** (metadata filter, vector search, keyword search, doc fetcher) | `eliza_rag` package has search + keyword_search; FASB has doc fetching; MCP server (`terraform/rag-pipeline/mcp_server/`) has V0 tools | **RESOLVED** — auth (#13) + ECS (#9) done |
| **Hybrid retriever with RRF** | `eliza_rag.retrieval.hybrid_retrieve()` already does RRF | Expose via MCP agent |
...
|-----------|---------------|--------|
| **Workspace MCP server** | Expose workspaces/applets as MCP servers so ChatGPT Enterprise (or any MCP client) can connect | Not started |
| **Standalone MCP server** | Version that works without the platform — for AWS-native deployments | **Done** — `terraform/rag-pipeline/mcp_server/` with auth + health check |
| **RAG core packaging** | Ensure RAG core can be imported standalone in any runtime | **Done** — `eliza-rag-ingestion` package |
| **API Gateway + WAF** | AWS API Gateway in front of standalone deployment | **Done** — `terraform/rag-pipeline/api_gateway.tf` |
...
Standalone `terraform/rag-pipeline/` project — provisions its own VPC, subnets,
IAM roles, security groups, S3 buckets, OpenSearch Serverless, and API Gateway.
MWAA handles worker autoscaling for 2TB+ workloads. Can be extracted to a
separate repo for customer deployments (zero platform dependencies).
...
|---|---|---|
| MWAA Orchestrator | **Terraform done** | `terraform/rag-pipeline/mwaa.tf`; DAGs V0 built (need fixes #2, #4) |
| SharePoint data source | **V0 built** | `dags/sharepoint_sync.py` + `dags/common/sharepoint_client.py` (need fixes #4, #6) |
| S3 Landing Zone | **Done** | `terraform/rag-pipeline/s3.tf` — raw, processed, DAGs buckets |
| Document Classifier (Fargate) | **V0 built** | `dags/document_pipeline.py` — classify task (need fix #7) |
| Route by Doc Metadata | **V0 built** | Router task in `document_pipeline.py` |
| Unstructured Text NLP Cleaning | **V0 built** | `process_unstructured` task |
| Structured Data Schema Validation | **V0 built** | `process_structured` task |
| PDF/Scanned OCR Extraction | **Done** | `eliza_rag.document_parser` VLM + GPT-4o parsers |
| Load Balancer → Embedding | **V0 built** | `dags/common/embedding_load_balancer.py` (need fixes #5, #14) |

### Embedding & Storage

| Component | Platform Status | Notes |
|---|---|---|
| Embedding Model (Fargate, scale up/down) | **Done** | `eliza_rag.embeddings` — Bedrock Titan v1+v2 with sync retry logic |
| S3 Processed Docs Store | **Done** | `terraform/rag-pipeline/s3.tf` — processed-documents bucket |
...
| Component | Platform Status | Notes |
|---|---|---|
| API Gateway + WAF | **Done** | `terraform/rag-pipeline/api_gateway.tf` — HTTP API + WAF with rate limiting |
...
### Overall: ~80% of the reference architecture is built. The remaining gaps are: DAG runtime fixes (triggering, auth, async), MCP server ECS deployment + auth, and MWAA operational wiring (requirements, DAG deployment CI/CD, Secrets Backend).
...
For clients who need a pure AWS stack they own and operate. Uses `eliza-rag-ingestion` as a Python package but doesn't deploy the platform itself.

```
cd terraform/rag-pipeline && terraform apply → provisions:
  ├─ VPC (10.1.0.0/16)       — isolated network
  ├─ MWAA (Airflow)           — orchestration
  ├─ S3                       — raw-documents, processed-documents, airflow-dags
  ├─ Bedrock                  — Titan embeddings + Claude generation (IAM)
  ├─ OpenSearch Serverless    — vector store (VPC endpoint)
  ├─ API Gateway + WAF        — rate limiting, auth
  └─ Secrets Manager          — SharePoint credentials

Code deployed:
  ├─ Airflow DAGs             — sharepoint_sync, document_pipeline
  ├─ eliza-rag-ingestion      — parse, chunk, embed, index (standalone package)
...
**What's done:**
1. RAG core packaged as `eliza-rag-ingestion` (no platform imports)
2. Standalone MCP server V0 (`terraform/rag-pipeline/mcp_server/`) with auth + health check
3. Terraform complete: `terraform/rag-pipeline/` (MWAA, S3, OpenSearch, API GW, VPC, IAM)
4. Airflow DAGs V0 built (SharePoint sync, document processing)

**What's remaining:**
1. Add MCP server ECS task definition to `terraform/rag-pipeline/`
2. Add auth + health check to MCP server
3. Fix DAG runtime issues (see fixes doc #2, #4, #5, #6)
4. Wire MWAA requirements + Secrets Backend
5. DAG deployment CI/CD pipeline
...
```
cd terraform/platform && terraform apply → full Eliza platform on ECS Fargate
...
```
shared/
  ├─ packages/eliza-rag-ingestion/  — parse, chunk, embed, index, retrieve (standalone package)
  ├─ src/services/fasb_service      — citations, reranking (platform-side, needs extraction)
  ├─ dags/                          — Airflow DAGs (identical files)
  └─ terraform/rag-pipeline/mcp_server/ — MCP server (standalone, co-located with infra)
```

Both paths install `eliza-rag-ingestion` as a dependency. Same package, same DAGs, same MCP server. Path A uses `terraform/rag-pipeline/`; Path B uses `terraform/platform/`.
...
1. ~~**RAG core packaging:**~~ Extracted to `packages/eliza-rag-ingestion/`.
2. ~~**MWAA Terraform:**~~ Standalone `terraform/rag-pipeline/` project.
3. ~~**Terraform split:**~~ `terraform/platform/` + `terraform/rag-pipeline/` (atomic, portable).
4. ~~**V0 SharePoint Sync DAG:**~~ `dags/sharepoint_sync.py`.
5. ~~**V0 Document Classifier:**~~ `dags/document_pipeline.py`.
6. ~~**V0 PPT/Table Preprocessors:**~~ Tasks in `document_pipeline.py`.
7. ~~**V0 Standalone MCP Server:**~~ `terraform/rag-pipeline/mcp_server/server.py` with auth + health check.
8. ~~**Self-hosted Airflow:**~~ Added to `docker-compose.yml`.
9. ~~**API Gateway + WAF:**~~ `terraform/rag-pipeline/api_gateway.tf`.

### Remaining

1. **DAG runtime fixes:** Fix triggering (#2), OAuth refresh (#4), async embed (#5), dedup (#6) — see `design_docs/AWS_RAG_PIPELINE_FIXES.md`.
2. **MCP server hardening:** Add auth + health check (#13), add ECS task definition to `terraform/rag-pipeline/` (#9).
3. **MWAA operational wiring:** Requirements upload (#12), Secrets Backend for SharePoint creds (#3), DAG deployment CI/CD (#15).
4. **Phase 6 smoke tests:** End-to-end validation across platform + Airflow + MCP runtimes.
5. **Workspace MCP Server:** Expose workspaces/applets as MCP endpoints for ChatGPT Enterprise.
6. **FASB extraction:** Extract citation/reranking pipeline from platform into `eliza-rag-ingestion` or a new standalone package.
