...
- **Fix recommendation**
  - Update `init_tenant.py` bootstrap to create tenant system roles (`admin/editor/viewer`) for initialized tenant(s), not only `platform_admin`.
  - Keep `platform_admin` excluded in invites, but ensure at least one assignable tenant role exists by default.
- **Likely affects other applets?** **Yes (high)**
...
- **Fix recommendation**
  - For tenant role creation context, use a strictly tenant-scoped endpoint (always filtered by tenant feature allocations), even for platform-admin users acting inside tenant admin UI.
  - Optionally require explicit "impersonate tenant-scoped view" mode for platform admins.
- **Likely affects other applets?** **Yes (high)**
...
- **Fix recommendation**
  - Add optional role filtering by tenant allocated features (similar to permission filtering strategy).
  - Decide policy for platform-admin context:
    - either keep global visibility only in true platform admin screens,
    - or enforce tenant-scoped view in tenant admin screens even for platform admins.
- **Likely affects other applets?** **Yes (high)**
...
2. Issue #5 - Missing default assignable tenant roles
3. Issue #6 - Create Role shows global permission/role surface
4. Issue #7 - Platform admin role surfaces over-broad in tenant context
5. Issue #3 - AI Recruiter first-load inconsistency
6. Issue #4 - Deferred (consolidation path)
7. Issue #2 - Last (teammate-owned workspace/upload flow)
...
- Applied submenu policy refinement:
  - **Base admin/platform items** are permission-gated (not applet page-gated),
  - **exceptions** remain page-gated (for example `data-connections`, AI Console pages, `Adoption Access`).
  - This prevents empty Admin/Platform panels in applet-scoped builds while keeping applet-specific entries controlled.
